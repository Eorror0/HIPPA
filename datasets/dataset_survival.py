...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        csv_path = 'dataset_csv/ccrcc_clean.csv', mode = 'path',cancer_type = 'blca',
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    return (path_features, label, event_time, c, slide_ids)

                elif self.mode == 'graph':
                    path_features = []
                    from datasets.BatchWSI import BatchWSI
                    for slide_id in slide_ids:
                        wsi_path = os.path.join(data_dir, 'pyg', '{}.pt'.format(slide_id.rstrip('.svs')))
                        wsi_bag = torch.load(wsi_path)
                        path_features.append(wsi_bag)
                    path_features = BatchWSI.from_data_list(path_features, update_cat_dims={'edge_latent': 1})
                    return (path_features, label, event_time, c, slide_ids)

                elif self.mode == 'supergraph':
                    path_features = []
                    from datasets.BatchWSI import BatchWSI
                    for slide_id in slide_ids:
                        wsi_path = os.path.join(data_dir, '{}_0.75_graph_torch.pt'.format(slide_id.rstrip('.svs').split('.')[0]))
                        wsi_bag = torch.load(wsi_path)
                        path_features.append(wsi_bag)
                    path_features = BatchWSI.from_data_list(path_features, update_cat_dims={'edge_latent': 1})
                    return (path_features, label, event_time, c, slide_ids)

                elif self.mode == 'super_pixel':
                    path_features = []
                    from datasets.BatchWSI import BatchWSI
                    for slide_id in slide_ids:
                        slide_id, _ = os.path.splitext(slide_id)
                        wsi_path = os.path.join(data_dir, slide_id, slide_id+'_dyclu.pt')
                        wsi_bag = torch.load(wsi_path)
                        path_features.append(wsi_bag)
                    path_features = BatchWSI.from_data_list(path_features, update_cat_dims={'edge_latent': 1})
                    return (path_features, label, event_time, c, slide_ids)
